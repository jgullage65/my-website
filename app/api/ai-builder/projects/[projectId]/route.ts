...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    const [crawlDiagnostics,generationDiagnostics]=await Promise.all([
      sql`SELECT status,attempt_number,started_at,completed_at,duration_ms,pages_discovered,pages_processed,pages_skipped,pages_failed,warnings,failure_stage FROM ai_builder_crawl_telemetry WHERE project_id=${normalizedProjectId} ORDER BY started_at DESC LIMIT 10`.catch(()=>[] as DatabaseRow[]),
      sql`SELECT status,attempt_number,started_at,completed_at,duration_ms,model,knowledge_count,faq_count,retry_count,input_tokens,output_tokens,total_tokens,failure_stage FROM ai_builder_generation_telemetry WHERE project_id=${normalizedProjectId} ORDER BY started_at DESC LIMIT 10`.catch(()=>[] as DatabaseRow[]),
