...
export async function GET(_request: Request, { params }: { params: { jobId: string } }) {
  ...
  return NextResponse.json({ ok: true, job: { id: job.id, state: job.state, pagesDiscovered: job.pagesDiscovered, pagesCrawled: job.pagesCrawled, crawlComplete: job.crawlComplete, processingPercent: job.processingPercent, attemptCount: job.attemptCount, nextAttemptAt: job.nextAttemptAt, result: job.state === "completed" ? job.result : null, errorMessage: job.errorMessage } });
}
