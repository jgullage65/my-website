...
function ServiceCard({ service }: { service: (typeof serviceGroups)[number] }) {
  return <motion.article variants={fadeUp} className="rounded-[1.35rem] border border-[rgba(212,175,55,.13)] bg-[linear-gradient(145deg,rgba(9,16,32,.94),rgba(2,5,14,.98))] p-6 shadow-[0_28px_80px_rgba(0,0,0,.34)]"><div className="flex flex-wrap items-center justify-center gap-4"><p className="text-xs font-black uppercase tracking-[.26em] text-[var(--gold)]">{service.eyebrow}</p>{service.badge ? <span className="rounded-full border border-[rgba(212,175,55,.24)] bg-[var(--gold)] px-3 py-1 text-xs font-black text-[#06101f]">{service.badge}</span> : null}</div><h3 className="mt-4 text-center text-2xl font-black tracking-[-.04em]">{service.title}</h3><p className="mt-3 text-center leading-7 text-[var(--muted)]">{service.subtitle}</p><div className="mt-5 rounded-[1rem] border border-[rgba(212,175,55,.12)] bg-white/[.035] p-4 text-center"><p className="text-sm font-black text-white">Best fit</p><p className="mt-1 text-sm leading-6 text-slate-300">{service.bestFor}</p></div><ul className="mx-auto mt-5 grid w-fit grid-cols-[.5rem_minmax(0,1fr)] gap-x-3 gap-y-2 text-left text-sm text-slate-200">{service.bullets.map((bullet) => <li key={bullet} className="contents"><span className="mt-1.5 h-2 w-2 rounded-full bg-[var(--gold)] shadow-[0_0_18px_rgba(212,175,55,.6)]" /><span>{bullet}</span></li>)}</ul><p className="mt-5 text-sm font-bold text-white">Outcome: <span className="font-normal text-[var(--muted)]">{service.outcome}</span></p><div className="mt-6 flex flex-col items-center gap-2">{service.actions.map((action) => { const classes = action.variant === "primary" ? "bg-[var(--gold)] text-[#06101f]" : action.variant === "link" ? "text-[var(--gold)] px-1" : "border border-[rgba(212,175,55,.18)] bg-white/[.035] text-slate-100 hover:bg-white/[.06]"; const content = <><span>{action.label}</span><span aria-hidden>→</span></>; return action.internal ? <Link key={action.label} href={action.href} className={`inline-flex w-fit items-center justify-center gap-2 whitespace-nowrap rounded-lg px-4 py-2 text-sm font-black transition ${classes}`}>{content}</Link> : <a key={action.label} href={action.href} target="_blank" rel="noreferrer" className={`inline-flex w-fit items-center justify-center gap-2 whitespace-nowrap rounded-lg px-4 py-2 text-sm font-black transition ${classes}`}>{content}</a>; })}</div></motion.article>;
}

export default function ServicesPageClient() {
  return <div className="overflow-hidden bg-[#030713] text-white"><Section className="relative mx-auto max-w-[94rem] px-5 py-10 sm:px-8 sm:py-12 lg:px-10 lg:py-14"><div className="absolute inset-x-0 top-0 h-[36rem] bg-[radial-gradient(circle_at_12%_18%,rgba(212,175,55,.16),transparent_28rem),radial-gradient(circle_at_82%_12%,rgba(14,22,62,.72),transparent_38rem)]" /><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(255,255,255,.035)_1px,transparent_1px),linear-gradient(180deg,rgba(255,255,255,.025)_1px,transparent_1px)] bg-[size:72px_72px] opacity-20" /><motion.div variants={fadeUp} className="relative z-10 mx-auto max-w-7xl text-center"><h1 className="text-4xl font-black leading-[1.02] tracking-[-.055em] sm:text-5xl lg:text-6xl xl:text-[4.75rem]">Premium websites, AI systems, and business technology <span className="text-[var(--gold)]">built around your goals.</span></h1><p className="mx-auto mt-5 max-w-3xl text-base leading-7 text-[var(--muted)] sm:text-lg sm:leading-8">Choose a focused service or combine strategy, design, automation, and custom development into one clear build plan. Every recommendation is scoped for practical business results.</p><div className="mt-7 flex flex-col items-center justify-center gap-3 sm:flex-row"><GoldButton href="/contact">Get a Recommendation</GoldButton><OutlineButton href="/pricing">View Pricing</OutlineButton></div></motion.div></Section><Section className="mx-auto max-w-[94rem] border-y border-[rgba(212,175,55,.10)] px-5 pb-10 pt-16 text-center sm:px-8 lg:px-10 lg:pb-12"><motion.p variants={fadeUp} className="text-xs font-black uppercase tracking-[.32em] text-[var(--gold)]">Service menu</motion.p><motion.h2 variants={fadeUp} className="mx-auto mt-4 max-w-4xl text-4xl font-black tracking-[-.055em] sm:text-6xl">Clear offers. Flexible combinations. <span className="text-[var(--gold)]">No tech overwhelm.</span></motion.h2><div className="mt-10 grid gap-5 lg:grid-cols-2">{serviceGroups.map((service) => <ServiceCard key={service.title} service={service} />)}</div></Section></div>;
}