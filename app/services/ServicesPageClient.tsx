...
function OutlineButton({ href, children }: { href: string; children: ReactNode }) {
  return <Link href={href} className="inline-flex w-fit items-center justify-center whitespace-nowrap rounded-lg border border-[rgba(212,175,55,.42)] bg-[#081226]/80 px-5 py-3 text-sm font-black text-white shadow-[inset_0_1px_0_rgba(255,255,255,.05)] transition duration-300 hover:-translate-y-0.5 hover:bg-white/[.06]">{children}</Link>;
}

function ServiceCard({ service }: { service: (typeof serviceGroups)[number] }) {
  return <motion.article variants={fadeUp} className="rounded-[1.35rem] border text-center border-[rgba(212,175,55,.13)] bg-[linear-gradient(145deg,rgba(9,16,32,.94),rgba(2,5,14,.98))] p-6 shadow-[0_28px_80px_rgba(0,0,0,.34)]"><div className="flex flex-wrap items-center justify-center gap-4"><p className="text-xs font-black uppercase tracking-[.26em] text-[var(--gold)]">{service.eyebrow}</p>{service.badge ? <span className="rounded-full border border-[rgba(212,175,55,.24)] bg-[var(--gold)] px-3 py-1 text-xs font-black text-[#06101f]">{service.badge}</span> : null}</div><h3 className="mt-4 text-2xl font-black tracking-[-.04em]">{service.title}</h3><p className="mt-3 leading-7 text-[var(--muted)]">{service.subtitle}</p><div className="mt-5 rounded-[1rem] border border-[rgba(212,175,55,.12)] bg-white/[.035] p-4"><p className="text-sm font-black text-white">Best fit</p><p className="mt-1 text-sm leading-6 text-slate-300">{service.bestFor}</p></div><ul className="mt-5 grid gap-2 text-sm text-slate-200">{service.bullets.map((bullet) => <li key={bullet} className="flex justify-center gap-3"><span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-[var(--gold)] shadow-[0_0_18px_rgba(212,175,55,.6)]" />{bullet}</li>)}</ul><p className="mt-5 text-sm font-bold text-white">Outcome: <span className="font-normal text-[var(--muted)]">{service.outcome}</span></p><div className="mt-6 flex flex-col items-center gap-2">{service.actions.map((action) => { const classes = action.variant === "primary" ? "bg-[var(--gold)] text-[#06101f]" : action.variant === "link" ? "text-[var(--gold)] px-1" : "border border-[rgba(212,175,55,.18)] bg-white/[.035] text-slate-100 hover:bg-white/[.06]"; const content = <><span>{action.label}</span><span aria-hidden>→</span></>; return action.internal ? <Link key={action.label} href={action.href} className={`inline-flex w-fit items-center justify-center gap-2 whitespace-nowrap rounded-lg px-4 py-2 text-sm font-black transition ${classes}`}>{content}</Link> : <a key={action.label} href={action.href} target="_blank" rel="noreferrer" className={`inline-flex w-fit items-center justify-center gap-2 whitespace-nowrap rounded-lg px-4 py-2 text-sm font-black transition ${classes}`}>{content}</a>; })}</div></motion.article>;
}

export default function ServicesPageClient() {
  return <div className="overflow-hidden bg-[#030713] text-white"><Section className="relative mx-auto max-w-[94rem] px-5 pb-16 pt-16 sm:px-8 lg:px-10 lg:pb-24 lg:pt-24"><div className="absolute inset-x-0 top-0 h-[52rem] bg-[radial-gradient(circle_at_12%_18%,rgba(212,175,55,.16),transparent_28rem),radial-gradient(circle_at_82%_12%,rgba(14,22,62,.72),transparent_38rem)]" /><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(255,255,255,.035)_1px,transparent_1px),linear-gradient(180deg,rgba(255,255,255,.025)_1px,transparent_1px)] bg-[size:72px_72px] opacity-20" /><motion.div variants={fadeUp} className="relative z-10 mx-auto max-w-5xl text-center"><p className="text-xs font-black uppercase tracking-[.34em] text-[var(--gold)]">Services</p><h1 className="mt-5 max-w-5xl text-5xl font-black leading-[.94] tracking-[-.065em] sm:text-6xl xl:text-[5.25rem]">Premium websites, AI systems, and business technology built around your goals.</h1><p className="mx-auto mt-6 max-w-3xl text-base leading-8 text-[var(--muted)] sm:text-lg">Choose a focused service or combine strategy, design, automation, and custom development into one clear build plan. Every recommendation is scoped for practical business results.</p><div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row"><GoldButton href="/contact">Get a Recommendation</GoldButton><OutlineButton href="/pricing">View Pricing</OutlineButton></div></motion.div></Section><Section className="mx-auto max-w-[94rem] border-y border-[rgba(212,175,55,.10)] px-5 py-20 text-center sm:px-8 lg:px-10"><motion.p variants={fadeUp} className="text-xs font-black uppercase tracking-[.32em] text-[var(--gold)]">Service menu</motion.p><motion.h2 variants={fadeUp} className="mx-auto mt-4 max-w-4xl text-4xl font-black tracking-[-.055em] sm:text-6xl">Clear offers. Flexible combinations. No tech overwhelm.</motion.h2><div className="mt-10 grid gap-5 lg:grid-cols-2">{serviceGroups.map((service) => <ServiceCard key={service.title} service={service} />)}</div></Section><Section className="mx-auto max-w-[94rem] px-5 py-10 text-center sm:px-8 lg:px-10"><motion.div variants={fadeUp} className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-[rgba(212,175,55,.18)] bg-[#050b18] p-6 shadow-[0_18px_50px_rgba(0,0,0,.28)] md:flex-row md:items-center"><div><p className="text-xs font-black uppercase tracking-[.28em] text-[var(--gold)]">Not sure what you need?</p><p className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-[var(--muted)]"><span className="font-black text-white">Send the goal.</span> I’ll recommend the cleanest service mix before you spend money.</p></div><div className="flex shrink-0 flex-col items-center justify-center gap-3 sm:flex-row"><GoldButton href="/contact">Contact Me</GoldButton><OutlineButton href="/payments">Payments</OutlineButton></div></motion.div></Section></div>;
}
