...
function ServiceCard({ service }: { service: (typeof serviceGroups)[number] }) {
  return <motion.article variants={fadeUp} className="rounded-[1.35rem] border border-[rgba(212,175,55,.13)] bg-[linear-gradient(145deg,rgba(9,16,32,.94),rgba(2,5,14,.98))] p-6 shadow-[0_28px_80px_rgba(0,0,0,.34)]"><div className="flex flex-wrap items-center justify-center gap-4"><p className="text-xs font-black uppercase tracking-[.26em] text-[var(--gold)]">{service.eyebrow}</p>{service.badge ? <span className="rounded-full border border-[rgba(212,175,55,.24)] bg-[var(--gold)] px-3 py-1 text-xs font-black text-[#06101f]">{service.badge}</span> : null}</div><h3 className="mt-4 text-center text-2xl font-black tracking-[-.04em]">{service.title}</h3><p className="mt-3 text-center leading-7 text-[var(--muted)]">{service.subtitle}</p><div className="mt-5 rounded-[1rem] border border-[rgba(212,175,55,.12)] text-left bg-white/[.035] p-4"><p className="text-sm font-black text-white">Best fit</p><p className="mt-1 text-sm leading-6 text-slate-300">{service.bestFor}</p></div><ul className="mt-5 grid gap-2 text-sm text-slate-200">{service.bullets.map((bullet) => <li key={bullet} className="flex gap-3"><span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-[var(--gold)] shadow-[0_0_18px_rgba(212,175,55,.6)]" />{bullet}</li>)}</ul><p className="mt-5 text-sm font-bold text-white">Outcome: <span className="font-normal text-[var(--muted)]">{service.outcome}</span></p><div className="mt-6 flex flex-col items-center gap-2">{service.actions.map((action) => { const classes = action.variant === "primary" ? "bg-[var(--gold)] text-[#06101f]" : action.variant === "link" ? "text-[var(--gold)] px-1" : "border border-[rgba(212,175,55,.18)] bg-white/[.035] text-slate-100 hover:bg-white/[.06]"; const content = <><span>{action.label}</span><span aria-hidden>→</span></>; return action.internal ? <Link key={action.label} href={action.href} className={`inline-flex w-fit items-center justify-center gap-2 whitespace-nowrap rounded-lg px-4 py-2 text-sm font-black transition ${classes}`}>{content}</Link> : <a key={action.label} href={action.href} target="_blank" rel="noreferrer" className={`inline-flex w-fit items-center justify-center gap-2 whitespace-nowrap rounded-lg px-4 py-2 text-sm font-black transition ${classes}`}>{content}</a>; })}</div></motion.article>;
}
