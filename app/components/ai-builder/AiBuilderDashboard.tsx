...
export default function AiBuilderDashboard({showcase=false}:{showcase?:boolean}){
 ...
 const manualKnowledge=session.contextEntries.filter(item=>(item.source.sourceType==="manual_intake"||item.source.sourceType==="user_edit")&&(item.status==="approved"||item.status==="corrected")).length,approvedFaq=session.faqEntries.filter(item=>item.status==="approved"||item.status==="corrected").length;
 ...
 return <div className={`space-y-5 pb-2 ${showcase?"h-full":""}`}>
  <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
   <Summary label="Project" value={project.businessName||"Untitled project"} detail={project.industry||"Industry not set"}/>
