    ...
    <div className="space-y-20 pb-8 sm:space-y-24">
          ...
          <article className="relative overflow-hidden rounded-[28px] border border-amber-300/25 bg-[linear-gradient(145deg,rgba(18,34,51,0.96),rgba(7,19,33,0.98))] p-6 shadow-[0_24px_90px_rgba(0,0,0,0.35),0_0_45px_rgba(245,158,11,0.06)] sm:p-8">
            ...
            <div className="relative">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${value.websiteKnowledge ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-300" : "border-white/10 bg-white/[0.04] text-slate-400"}`}>
              ...
              {value.websiteKnowledge ? (
                <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3">
                  <Metric label="Pages imported" value={String(value.websiteKnowledge.pages.length)} />
                  <Metric label="Warnings" value={String(value.websiteKnowledge.warnings.length)} />
                  <Metric
                    label="Last updated"
                    value={new Date(value.websiteKnowledge.importedAt).toLocaleDateString()}
      ...
      <section>
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-2">
          <KnowledgeCard icon="◈" title="Business profile" description="The identity your assistant represents.">
          ...
          <KnowledgeCard icon="✦" title="Products & Services" description="What you sell, how it works, and why customers choose it.">
            <textarea
              rows={9}
              className={`${inputClassName} resize-y`}
              placeholder={value.websiteKnowledge?.productsServices ? "Add private details, corrections, packages, pricing, or anything your website does not explain." : "Describe your services, packages, deliverables, pricing structure, and what each option is for."}
              value={value.userKnowledge.productsServices}
              onChange={(event) => updateUserKnowledge("productsServices", event.target.value)}
            />
          </KnowledgeCard>

          <KnowledgeCard icon="◎" title="Ideal Customers" description="Who your AI should recognize as the best fit.">
            <textarea
              rows={8}
              className={`${inputClassName} resize-y`}
              placeholder={value.websiteKnowledge?.idealCustomers ? "Add more specific customer details or correct anything the website got wrong." : "Describe your best-fit customers, industries, company sizes, locations, needs, and common goals."}
              value={value.userKnowledge.idealCustomers}
              onChange={(event) => updateUserKnowledge("idealCustomers", event.target.value)}
            />
          </KnowledgeCard>

          <KnowledgeCard icon="☰" title="Additional Business Knowledge" description="Private policies, processes, objections, FAQs, and operating knowledge.">
            <textarea
              rows={8}
              className={`${inputClassName} resize-y`}
              ...
              onChange={(event) => updateUserKnowledge("additionalKnowledge", event.target.value)}
            />
          </KnowledgeCard>
        </div>

        <div className="mt-5 rounded-[28px] border border-white/10 bg-white/[0.035] p-6 sm:p-8">
          <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr] lg:items-center">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-400">
                Choose the default personality. You can refine this later as the
                assistant learns your voice.
              </p>
            </div>
            <select
              className={inputClassName}
      ...
      <section className="relative overflow-hidden rounded-[34px] border border-amber-300/25 bg-[linear-gradient(145deg,rgba(19,35,52,0.98),rgba(6,16,31,0.98))] px-6 py-14 text-center shadow-[0_30px_100px_rgba(0,0,0,0.42),0_0_70px_rgba(245,158,11,0.07)] sm:px-10 sm:py-20">
...
function SectionHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) {
  return (
    <div className="mx-auto max-w-3xl text-center">
      ...
      <h2 className="mt-3 text-3xl font-semibold tracking-[-0.035em] text-white sm:text-5xl">{title}</h2>
...
function Metric({ label, value, className = "" }: { label: string; value: string; className?: string }) {
  return (
    <div className={`rounded-2xl border border-white/[0.07] bg-black/10 p-4 ${className}`}>
  ...
  return (
    <article className="rounded-[28px] border border-white/10 bg-[linear-gradient(145deg,rgba(16,31,47,0.85),rgba(7,19,33,0.92))] p-6 shadow-[0_20px_70px_rgba(0,0,0,0.22)] sm:p-8">
      <div className="flex items-start gap-4">
        <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-amber-300/15 bg-amber-300/[0.07] font-semibold text-amber-300">{icon}</div>
        <div>
...
function Status({ tone, children }: { tone: "success" | "error"; children: ReactNode }) {
  return (
    <div className={`mt-4 rounded-xl border px-4 py-3 text-sm ${tone === "success" ? "border-emerald-400/20 bg-emerald-400/[0.07] text-emerald-200" : "border-red-400/20 bg-red-400/[0.07] text-red-200"}`}>
      {children}
    </div>
  );
}
...
function WebsiteKnowledgeModal({ knowledge, onClose }: { knowledge: WebsiteKnowledge; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#02060d]/80 p-0 backdrop-blur-md sm:items-center sm:p-6" onMouseDown={onClose}>
      <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-t-[30px] border border-white/10 bg-[#081522] shadow-2xl sm:rounded-[30px]" onMouseDown={(event) => event.stopPropagation()}>
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/[0.07] bg-[#081522]/95 px-5 py-5 backdrop-blur sm:px-8">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-amber-300">Website knowledge</p>
            <h2 className="mt-1 text-2xl font-semibold text-white">Imported business information</h2>
          </div>
          <button type="button" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-full border border-white/10 bg-white/[0.04] text-xl text-slate-300 hover:bg-white/[0.08]">×</button>
        </div>
        <div className="grid gap-5 p-5 sm:p-8">
          <ReadOnlyBlock title="Company Summary" content={[knowledge.businessName && `Business: ${knowledge.businessName}`, knowledge.industry && `Industry: ${knowledge.industry}`, knowledge.website && `Website: ${knowledge.website}`].filter(Boolean).join("\n")} />
...
function ReadOnlyBlock({ title, content }: { title: string; content: string }) {
  return (
    <section className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-5">
      ...
      <p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-slate-300">{content || "No information found."}</p>
    </section>
  );
}
