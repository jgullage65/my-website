...
export default function AiBuilderModelSelect({models,value,disabled,onChange,className="",defaultOpen=false}:{models:AiBuilderModelChoice[];value:string;disabled:boolean;onChange:(modelId:string)=>void;className?:string;defaultOpen?:boolean}) {
  ...
  async function selectModel(model: AiBuilderModelChoice) {
    ...
    if (model.provider === "anthropic") {
      await showConfirm({
        title: "Claude is available for conversational AI",
        message: "To ensure complete and reliable Business Memory generation, Claude models are reserved for conversational AI and are not available for knowledge pack creation at this time.",
        confirmLabel: "Got it",
        hideCancel: true,
      });
      return;
  ...
  return (
    <div ref={rootRef} className={`relative grid min-w-0 justify-items-center gap-1.5 ${className}`}>
      ...
      {open ? <div role="listbox" aria-label="Available AI models" className="absolute left-1/2 top-full z-[100] mt-2 max-h-[min(28rem,65vh)] w-[min(21rem,calc(100vw-3rem))] -translate-x-1/2 overflow-y-auto rounded-xl border border-amber-300/20 bg-[#050505] p-2 shadow-[0_24px_70px_rgba(0,0,0,.75)]">
