    ...
    <AiBuilderShell>
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div><p className="text-xs font-black uppercase tracking-[.3em] text-[var(--gold)]">AI Builder</p><h1 className="mt-3 text-3xl font-black sm:text-5xl">Your Projects</h1><p className="mt-3 text-slate-400">Continue building and refining your business AI systems.</p></div>
          <Link href="/ai-builder?new=1" className="inline-flex justify-center rounded-xl bg-[var(--gold)] px-5 py-3 text-sm font-black text-[#071027] shadow-[0_12px_32px_rgba(212,175,55,.2)] transition hover:-translate-y-0.5">New AI Builder Project</Link>
        </div>

        {error ? <div role="alert" className="mt-7 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-200">{error}</div> : null}
        ...
        {!loading && !projects.length ? <div className="mt-10 rounded-[30px] border border-amber-300/20 bg-[#030713] px-6 py-16 text-center shadow-[0_24px_90px_rgba(0,0,0,.34)]"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-amber-300/30 bg-amber-300/10 text-2xl text-[var(--gold)]">✦</div><h2 className="mt-6 text-2xl font-black">Build your first business AI</h2><p className="mx-auto mt-3 max-w-lg text-slate-400">Create a project and your work will be saved here whenever you return.</p><Link href="/ai-builder?new=1" className="mt-7 inline-flex rounded-xl bg-[var(--gold)] px-6 py-3 text-sm font-black text-[#071027]">Create Your First AI Builder Project</Link></div> : null}
        ...
        <div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {visible.map((project) => <article key={project.id} className="group relative rounded-3xl border border-amber-300/20 bg-[linear-gradient(145deg,rgba(8,18,38,.96),rgba(3,7,19,.96))] p-6 shadow-[0_20px_60px_rgba(0,0,0,.28)] transition hover:-translate-y-1 hover:border-amber-300/40">
            ...
            <Link href={`/ai-builder?projectId=${encodeURIComponent(project.id)}`} className="mt-5 flex justify-center rounded-xl border border-amber-300/25 bg-amber-300/5 px-4 py-2.5 text-sm font-black text-amber-200 hover:bg-amber-300/10">Open Project</Link>
          </article>)}
        </div>
      </div>
    </AiBuilderShell>
