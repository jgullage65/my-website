...
export default function AiBuilderProjects() {
  ...
  return (
    <AiBuilderShell>
      <div className="relative left-1/2 -my-10 min-h-screen w-screen -translate-x-1/2 bg-[#030713] px-4 py-10 sm:-my-14 sm:px-6 sm:py-14 lg:-my-20 lg:px-8 lg:py-20">
      <div className="mx-auto max-w-6xl">
        ...
        {!loading && projects.length ? <div className="mt-9 flex flex-col gap-3 sm:flex-row">
          <input aria-label="Search projects" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search business, website, or industry" className="min-w-0 flex-1 rounded-xl border border-white/[0.08] bg-[#07101f] px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-500 focus:border-amber-300/30" />
          <select aria-label="Sort projects" value={sort} onChange={(event) => setSort(event.target.value as Sort)} className="rounded-xl border border-white/[0.08] bg-[#07101f] px-4 py-3 text-sm text-slate-200 outline-none focus:border-amber-300/30"><option value="updated">Last Updated</option><option value="created">Created Date</option><option value="name">Business Name</option></select>
        </div> : null}
        ...
        <div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {visible.map((project) => <article key={project.id} className="group relative rounded-3xl border border-white/[0.08] bg-[#07101f] p-6 transition-colors hover:border-amber-300/25">
            ...
            <Link href={`/ai-builder?projectId=${encodeURIComponent(project.id)}`} className="block pt-5 text-center"><h2 className="mx-auto max-w-[85%] text-2xl font-black tracking-[-.035em] text-white group-hover:text-amber-100">{project.businessName}</h2><p className="mt-3 truncate text-sm text-slate-400">{project.website || "No website added"}</p><p className="mx-auto mt-5 line-clamp-3 max-w-sm text-sm font-medium leading-6 text-slate-300">{project.industry}</p><dl className="mt-6 space-y-2 border-t border-white/10 pt-4 text-xs text-slate-400"><div className="flex justify-between gap-3"><dt>Updated</dt><dd className="text-right text-slate-300">{date(project.updatedAt)}</dd></div><div className="flex justify-between gap-3"><dt>Created</dt><dd className="text-right text-slate-300">{date(project.createdAt)}</dd></div><div className="flex justify-between gap-3"><dt>Chat messages</dt><dd className="font-bold text-slate-300">{project.messageCount}</dd></div></dl></Link>
          </article>)}
        </div>
      </div>
      </div>
