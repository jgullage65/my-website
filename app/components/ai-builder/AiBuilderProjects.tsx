...
export default function AiBuilderProjects() {
  ...
  async function rename(project: Project) {
    const businessName = window
      .prompt("Rename project", project.businessName)
      ?.trim();
    ...
    try {
      const response = await fetch(
        `/api/ai-builder/projects/${encodeURIComponent(project.id)}`,
        {
          method: "PATCH",
      ...
      setProjects((current) =>
        current.map((item) =>
          item.id === project.id
            ? { ...item, businessName, updatedAt: new Date().toISOString() }
            : item,
        ),
      );
    } catch (reason) {
  ...
  async function archive(project: Project) {
    const confirmed = await showConfirm({
      ...
      message:
        "This removes the project from your active projects without deleting its saved knowledge or chat history.",
    ...
    <AiBuilderShell>
      ...
      <div className="relative mx-auto max-w-6xl rounded-[30px] border border-white/[0.09] bg-[#030713] px-4 py-8 shadow-[0_18px_60px_rgba(0,0,0,0.2)] sm:px-6 sm:py-10">
        ...
        <div className="text-center">
          <p className="text-xs font-black uppercase tracking-[.3em] text-[var(--gold)]">
            AI Builder
          </p>
          <h1 className="mt-3 text-3xl font-black tracking-[-.045em] text-white sm:text-5xl">
            Your <span className="text-[var(--gold)]">Projects</span>
          </h1>
        ...
        {loading ? (
          <div className="mt-10 rounded-[30px] border border-amber-300/20 bg-[#030713] p-12 text-center text-slate-400">
        ...
        {isSignedIn && !loading && !projects.length ? (
          <div className="mt-10 rounded-[30px] border border-amber-300/20 bg-[#030713] px-6 py-16 text-center shadow-[0_24px_90px_rgba(0,0,0,.34)]">
            ...
            <p className="mx-auto mt-3 max-w-lg text-slate-400">
              Create a project and your work will be saved here whenever you
              return.
            </p>
            <Link
              href="/ai-builder?new=1"
              className="mt-7 inline-flex items-center justify-center rounded-lg border border-amber-300/15 bg-[#081226] px-5 py-3 text-sm font-black text-white shadow-[0_18px_48px_rgba(212,175,55,.24),inset_0_1px_0_rgba(255,255,255,.55)] transition duration-300 hover:-translate-y-0.5 hover:border-amber-300/30 hover:bg-[#0b1830]"
            >
              Create Your First AI Builder Project
            </Link>
          </div>
        ) : null}
        <div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {projects.map((project) => (
            <article
              key={project.id}
              className="group relative rounded-3xl border border-amber-300/25 bg-[#030713] p-6 transition-colors hover:border-amber-300/40"
            >
              <div className="absolute right-4 top-4">
                <div className="relative">
                  <button
                    type="button"
                    aria-label={`Actions for ${project.businessName}`}
                    aria-haspopup="menu"
                    aria-expanded={menu === project.id}
                    onClick={() =>
                      setMenu(menu === project.id ? null : project.id)
                    }
                    className="flex h-9 w-9 items-center justify-center rounded-lg border border-amber-300/15 bg-[#030713] text-xl text-slate-300 hover:border-amber-300/30"
                  >
                    •••
                  </button>
                  {menu === project.id ? (
                    <div
                      role="menu"
                      className="absolute right-0 top-11 z-20 min-w-[160px] rounded-xl border border-[rgba(212,175,55,.2)] bg-[#030713] p-1.5 shadow-2xl"
                    >
                      <Link
                        role="menuitem"
                        href={`/ai-builder?projectId=${encodeURIComponent(project.id)}`}
                        className="block rounded-lg px-3 py-2 text-xs font-semibold text-slate-200 hover:bg-white/[0.04] hover:text-[var(--gold)]"
                      >
                        Open
                      </Link>
                      <button
                        ...
                        className="block w-full rounded-lg px-3 py-2 text-left text-xs font-semibold text-slate-200 hover:bg-white/[0.04] hover:text-[var(--gold)]"
                      >
                        Rename
                      </button>
                      <button
              ...
              <Link
                href={`/ai-builder?projectId=${encodeURIComponent(project.id)}`}
                className="block pt-5 text-center"
              >
                <h2 className="mx-auto max-w-[85%] text-2xl font-black tracking-[-.035em] text-[var(--gold)]">
                  {project.businessName}
                </h2>
                <p className="mt-3 truncate text-sm text-slate-400">
                  {project.website || "No website added"}
                </p>
                <p className="mx-auto mt-5 line-clamp-3 max-w-sm text-sm font-medium leading-6 text-slate-300">
                  {project.industry}
                </p>
                <dl className="mt-6 space-y-2 border-t border-white/10 pt-4 text-xs text-slate-400">
