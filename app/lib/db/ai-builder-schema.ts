...
async function createAiBuilderSchema() {
  ...
  await sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ai_builder_assistant_projection_parity_reports_active_runtime_authority_check') THEN ALTER TABLE ai_builder_assistant_projection_parity_reports ADD CONSTRAINT ai_builder_assistant_projection_parity_reports_active_runtime_authority_check CHECK (active_runtime_authority IN ('legacy', 'canonical')); END IF; END $$`;
  ...
  await sql`ALTER TABLE ai_builder_downstream_synchronization_jobs ADD COLUMN IF NOT EXISTS pending_trigger_type TEXT`;
  await sql`ALTER TABLE ai_builder_downstream_synchronization_jobs ADD COLUMN IF NOT EXISTS pending_actor_type TEXT`;
  await sql`ALTER TABLE ai_builder_downstream_synchronization_jobs ADD COLUMN IF NOT EXISTS pending_actor_id TEXT`;
  await sql`ALTER TABLE ai_builder_downstream_synchronization_jobs ADD COLUMN IF NOT EXISTS pending_command_id TEXT`;
  await sql`ALTER TABLE ai_builder_downstream_synchronization_jobs ADD COLUMN IF NOT EXISTS pending_reason TEXT`;
  ...
  await sql`ALTER TABLE ai_builder_downstream_synchronization_attempts ADD COLUMN IF NOT EXISTS execution_stage TEXT NOT NULL DEFAULT 'claimed'`;
  await sql`ALTER TABLE ai_builder_downstream_synchronization_attempts ADD COLUMN IF NOT EXISTS work_started BOOLEAN NOT NULL DEFAULT FALSE`;
  await sql`ALTER TABLE ai_builder_downstream_synchronization_attempts ADD COLUMN IF NOT EXISTS reason TEXT`;
  ...
  // Phase 12C: this is intentionally an append-only audit stream.  Consumers
  // derive current status from durable domain tables; events are never mutated.
  await sql`CREATE TABLE IF NOT EXISTS ai_builder_operational_events (
    id TEXT PRIMARY KEY,
    project_id TEXT NOT NULL REFERENCES ai_builder_projects(id) ON DELETE CASCADE,
    event_type TEXT NOT NULL,
    category TEXT NOT NULL,
    ...
    correlation_id TEXT,
    command_id TEXT,
    synchronization_job_id TEXT,
    synchronization_attempt_id TEXT,
    migration_run_id TEXT,
    trusted_knowledge_revision INTEGER,
    business_memory_revision INTEGER,
    assistant_projection_fingerprint TEXT,
    expected_value TEXT,
    observed_value TEXT,
    error_code TEXT,
    error_message TEXT,
