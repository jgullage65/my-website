...
export const safePublicUrl=(value:string)=>{try{const url=new URL(value);url.username="";url.password="";url.search="";url.hash="";return url.toString().slice(0,500);}catch{return "[invalid URL]";}};
...
const safeDiagnostics=(diagnostics:JsonItem|undefined)=>{if(!diagnostics)return null;const result:JsonItem={};for(const [key,value] of Object.entries(diagnostics)){if(typeof value==="number"&&Number.isFinite(value))result[key]=value;else if(key==="finalUrls"&&Array.isArray(value))result[key]=value.filter((item):item is string=>typeof item==="string").map(safePublicUrl);else if(key==="restrictions"&&Array.isArray(value))result[key]=safeRestrictions(value as CrawlRestriction[]);else if(key==="warningDetails"&&Array.isArray(value))result[key]=(value as JsonItem[]).map(item=>({stage:redact(item.stage),message:redact(item.message),url:safePublicUrl(String(item.url??""))}));else if(key==="timings"&&value&&typeof value==="object"&&!Array.isArray(value))result[key]=Object.fromEntries(Object.entries(value).filter((entry):entry is [string,number]=>typeof entry[1]==="number"&&Number.isFinite(entry[1])));}return result;};
...
export async function startCrawlTelemetry(id:string,requestedUrl:string,startedAt:string){
  await safely("AI_BUILDER_CRAWL_TELEMETRY_START_FAILED",async()=>{const sql=getSql();await sql`
    ...
    VALUES (${id},${safePublicUrl(requestedUrl)},'running',(SELECT COUNT(*)::integer+1 FROM ai_builder_crawl_telemetry WHERE requested_url=${safePublicUrl(requestedUrl)}),${startedAt}::timestamptz)
    ON CONFLICT (id) DO NOTHING`;});
}

export async function finishCrawlTelemetry(id:string,data:{status:string;resolvedUrl?:string;startedAt:string;completedAt:string;pagesDiscovered?:number;pagesProcessed?:number;pagesSkipped?:number;pagesFailed?:number;finalUrls?:string[];warnings?:JsonItem[];errors?:JsonItem[];restrictions?:CrawlRestriction[];diagnostics?:JsonItem;failureStage?:string}){
  await safely("AI_BUILDER_CRAWL_TELEMETRY_FINISH_FAILED",async()=>{const sql=getSql();await sql`
    UPDATE ai_builder_crawl_telemetry SET status=${data.status},resolved_url=${data.resolvedUrl?safePublicUrl(data.resolvedUrl):null},started_at=${data.startedAt}::timestamptz,completed_at=${data.completedAt}::timestamptz,
      ...
      final_urls=${JSON.stringify((data.finalUrls??[]).map(safePublicUrl))}::jsonb,warnings=${JSON.stringify(safeItems(data.warnings))}::jsonb,
      errors=${JSON.stringify(safeItems(data.errors))}::jsonb,restrictions=${data.restrictions?.length?JSON.stringify(safeRestrictions(data.restrictions)):null}::jsonb,
      crawler_diagnostics=${JSON.stringify(safeDiagnostics(data.diagnostics))}::jsonb,failure_stage=${data.failureStage??null} WHERE id=${id}`;});
}
