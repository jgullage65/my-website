...
test("deterministic fact titles never come from arbitrary chopped sentence prefixes", () => {
  const claim = "Dr. Kim has been treating and helping patients utilizing his unique approaches for over a decade.";
  const fact = website("about", [claim], "Provider Experience").find(f => f.category === "competitive_differentiator");
  assert.ok(fact);
  assert.equal(fact?.title, "Provider Experience");
  assert.equal(fact?.value, claim);
  assert.equal(fact?.title.includes(claim.slice(0, 20)), false);
});

test("privacy and tracking language cannot become commercial services", () => {
  const facts = website("policies", [
    "To personalize and develop our Services and the services we provide to you, we may collect device information.",
    "Not all browsers offer a DNT option and DNT signals are not yet uniform.",
  ], "Privacy Policy");
  assert.equal(facts.some(f => f.category === "service"), false);
  assert.equal(facts.some(f => f.category === "pricing_plan"), false);
  assert.equal(facts.some(f => f.category === "location_service_area"), false);
});

test("customer accident instructions cannot become business service areas", () => {
  const facts = website("services", [
    "After an accident, identify the other party's name, address, phone number, and insurance card.",
  ], "Auto Accident Guidance");
  assert.equal(facts.some(f => f.category === "location_service_area"), false);
});

test("commercial facts keep complete evidence values while using semantic labels", () => {
  const offer = "New Patient Special: Day 1 consultation, exam, and X-rays plus Day 2 report of findings and first session for $49.";
  const service = "Our clinic specializes in non-surgical spinal decompression therapy for unresolved neck and back pain.";
  const facts = website("services", [offer, service], "Services");
  const pricing = facts.find(f => f.category === "pricing_plan");
  const serviceFact = facts.find(f => f.category === "service" && /spinal decompression/i.test(f.value));
  assert.equal(pricing?.value, offer);
  assert.equal(serviceFact?.value, service);
  assert.equal(pricing?.title, "Pricing or offer");
  assert.equal(serviceFact?.title, "Service");
});
