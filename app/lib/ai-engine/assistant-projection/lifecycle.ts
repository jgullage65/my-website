...
import { safeOperationalError, writeOperationalEvent, writeOperationalFailureAfterRollback } from "@/app/lib/ai-engine/operations/operational-events";
...
async function rebuildAssistantProjectionAfterProjectLock(input: { client: QueryClient; projectId: string; forceRebuild?: boolean }): Promise<RebuildAssistantProjectionResult> {
  ...
  await writeOperationalEvent(input.client, { projectId: input.projectId, eventType: "assistant_projection_rebuild_started", category: "assistant_projection", severity: "info", outcome: "started", sourceComponent: "rebuildAssistantProjectionInTransaction", assistantProjectionFingerprint: projection.businessMemoryFingerprint, metadata: { previousInvalidationState: previousState, forceRebuild: Boolean(input.forceRebuild) } });
  ...
  try {
    ...
    await writeOperationalEvent(input.client, { projectId: input.projectId, eventType: "assistant_projection_rebuild_succeeded", category: "assistant_projection", severity: "info", outcome: "succeeded", sourceComponent: "rebuildAssistantProjectionInTransaction", assistantProjectionFingerprint: record.businessMemoryFingerprint, metadata: { projectionVersion: record.projectionVersion, schemaVersion: record.schemaVersion, previousInvalidationState: previousState, forceRebuild: Boolean(input.forceRebuild) } });
...
export async function rebuildAssistantProjectionForProject(input: { projectId: string; clerkUserId: string }): Promise<RebuildAssistantProjectionResult> {
  ...
  catch (cause) { failure = cause instanceof AssistantProjectionRebuildError ? cause : null; await client.query("ROLLBACK").catch(() => undefined); if (!failure?.failureContext) { const safe = safeOperationalError(cause); await writeOperationalFailureAfterRollback({ projectId: input.projectId, eventType: "assistant_projection_rebuild_failed", category: "assistant_projection", severity: "error", outcome: "failed", sourceComponent: "rebuildAssistantProjectionForProject", errorCode: safe.errorCode, errorMessage: safe.errorMessage }, cause); throw cause; } try { await client.query("BEGIN"); const owner = (await client.query("SELECT clerk_user_id FROM ai_builder_projects WHERE id=$1 FOR UPDATE", [input.projectId])).rows[0] as { clerk_user_id?: string } | undefined; if (owner?.clerk_user_id === input.clerkUserId) await markAssistantProjectionFailedIfUnchanged(client, { projectId: input.projectId, expectedFingerprint: failure.failureContext.previousFingerprint, expectedGeneratedAt: failure.failureContext.previousGeneratedAt, expectedUpdatedAt: failure.failureContext.previousUpdatedAt, allowedStates: [failure.failureContext.previousState, "rebuilding"] }); await client.query("COMMIT"); } catch (secondary) { await client.query("ROLLBACK").catch(() => undefined); (cause as Error & { failureRecordingError?: unknown }).failureRecordingError = secondary; } const safe = safeOperationalError(cause); await writeOperationalFailureAfterRollback({ projectId: input.projectId, eventType: "assistant_projection_rebuild_failed", category: "assistant_projection", severity: "error", outcome: "failed", sourceComponent: "rebuildAssistantProjectionForProject", assistantProjectionFingerprint: failure.failureContext.previousFingerprint, errorCode: safe.errorCode, errorMessage: safe.errorMessage }, cause); throw cause; }
