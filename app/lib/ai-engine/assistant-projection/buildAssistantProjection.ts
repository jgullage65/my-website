...
import {
  ...
  type AssistantProjectionProduct,
...
export function buildAssistantProjection(memory: BusinessMemory): AssistantProjection {
  ...
  const relationships = byId(memory.relationships.flatMap((relationship): AssistantProjectionRelationship[] => {
    ...
    if (!from || !to || !supportedAssertion(from) || !supportedAssertion(to) || from.entityId !== relationship.fromEntityId || to.entityId !== relationship.toEntityId || canonicalEntityId(relationship.fromEntityId) === null || canonicalEntityId(relationship.toEntityId) === null || !["service", "product", "pricing_concept", "policy", "faq"].includes(entities.get(relationship.fromEntityId)?.type ?? "") || !["service", "product", "pricing_concept", "policy", "faq"].includes(entities.get(relationship.toEntityId)?.type ?? "") || !relationship.sourceEntryIds.every((id) => typeof id === "string" && id.length > 0)) return [];
  ...
  const projection: AssistantProjection = { projectId: memory.projectId, businessMemoryFingerprint: businessMemoryFingerprint(memory), projectionVersion: ASSISTANT_PROJECTION_VERSION, schemaVersion: ASSISTANT_PROJECTION_SCHEMA_VERSION,
    ...
    // The v3 service text collection is the compatibility bucket for approved
    // canonical knowledge without a dedicated runtime collection. Entity type
    // and canonical section tags remain intact for retrieval and citations.
    services: byId(["service", "business", "customer_segment", "location", "process", "contact_method", "differentiator", "guarantee", "general_knowledge", "other"].flatMap((type) => ofType(type as import("../business-memory/contracts").BusinessEntityType))) as AssistantProjectionService[], products: ofType("product") as AssistantProjectionProduct[], pricing: ofType("pricing_concept") as AssistantProjectionPricing[], policies: ofType("policy") as AssistantProjectionPolicy[], faqs: byId(ofType("faq").map((item) => ({ ...item, question: item.title, answer: item.value }))) as AssistantProjectionFaq[], restrictions, relationships,
