...
function harness(status = "MATCH", fingerprint = projection.businessMemoryFingerprint) {
  ...
  const client = { release: () => { released++; }, query: async (sql: string) => { calls.push(sql); if (sql.includes("FROM ai_builder_projects") && sql.includes("FOR UPDATE")) return { rows: [{ id: "project-1" }] }; if (sql.includes("FROM ai_builder_assistant_projections")) return { rows: [{ project_id: "project-1", business_memory_fingerprint: projection.businessMemoryFingerprint, projection_version: 1, schema_version: 1, generated_at: "2026-01-01T00:00:00.000Z", invalidation_state: "valid", projection_json: projection, created_at: "2026-01-01T00:00:00.000Z", updated_at: "2026-01-01T00:00:00.000Z" }] }; if (sql.includes("parity_reports")) return { rows: [{ status, assistant_projection_version: 1, assistant_projection_schema_version: 1, active_runtime_authority: "canonical", artifact_fingerprint: fingerprint, compared_at: "2026-01-01T00:00:00.000Z" }] }; return { rows: [] }; } };
