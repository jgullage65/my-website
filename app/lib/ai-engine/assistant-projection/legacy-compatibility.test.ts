...
function projection(): AssistantProjection {
  ...
  return {
    ...
    restrictions: [{ id: "rule", type: "behavior_rule", instruction: "Be accurate.", relatedEntityIds: [], relatedAssertionIds: [], evidenceIds: [] }, { id: "claim", type: "prohibited_claim", instruction: "Do not promise outcomes.", relatedEntityIds: [], relatedAssertionIds: [], evidenceIds: [] }],
...
test("maps services, pricing, policies, FAQs, assistant configuration, sources, and supported restrictions", () => {
  ...
  assert.equal(adapted.facts.some((item) => item.id === "missing"), false); assert.equal(adapted.builtAt, "");
});
...
test("throws narrow errors for corrupt required data", () => {
  ...
  assert.throws(() => buildLegacyKnowledgePackFromAssistantProjection(invalidFact), (error: unknown) => error instanceof AssistantProjectionRuntimeValidationError && error.code === "assistant_projection_runtime_invalid_knowledge");
  const invalidFaq = projection(); invalidFaq.faqs[0].answer = "";
  assert.throws(() => buildLegacyKnowledgePackFromAssistantProjection(invalidFaq), (error: unknown) => error instanceof AssistantProjectionRuntimeValidationError && error.code === "assistant_projection_runtime_invalid_faq");
});
