...
const artifact = { projectionVersion: 1, schemaVersion: 1, generatedAt: "2026-01-01T00:00:00.000Z", businessMemoryFingerprint: "business_memory_abcdef0123456789abcdef01", invalidationState: "valid" };
const evidence = { status: "MATCH", projectionVersion: 1, schemaVersion: 1, activeRuntimeAuthority: "canonical", comparedAt: "2026-01-01T00:00:00.000Z", artifactFingerprint: artifact.businessMemoryFingerprint };
...
test("cutover evidence is bound to the current projection artifact", () => {
  ...
  assert.equal(check({ evidence: { ...evidence, artifactFingerprint: undefined } }), "assistant_projection_runtime_unavailable_parity_evidence_fingerprint_mismatch");
  assert.equal(check({ evidence: { ...evidence, artifactFingerprint: "business_memory_0123456789abcdef01234567" } }), "assistant_projection_runtime_unavailable_parity_evidence_fingerprint_mismatch");
