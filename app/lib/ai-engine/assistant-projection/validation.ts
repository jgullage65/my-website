...
export function validateAssistantProjectionRuntime(projection: AssistantProjection): void {
  ...
  for (const restriction of projection.restrictions) {
    if (!restriction || typeof restriction !== "object" || !["behavior_rule", "prohibited_claim"].includes(restriction.type) || !authoritativeStates.has(restriction.reviewState) || !Array.isArray(restriction.sourceIds) || restriction.sourceIds.some(id => !sourceIds.has(id)) || !Array.isArray(restriction.evidenceIds) || restriction.evidenceIds.some(id => !evidenceIds.has(id))) fail("assistant_projection_runtime_invalid_restriction");
  }
