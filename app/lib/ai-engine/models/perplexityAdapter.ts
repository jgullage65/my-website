...
export type AdapterInput={model:ModelDefinition;messages:{role:"user"|"assistant";content:string}[];instructions?:string;maxOutputTokens:number;signal?:AbortSignal;timeoutMs:number};
...
export async function runPerplexity(input:AdapterInput) {
  ...
  try {
    const response=await fetch("https://api.perplexity.ai/v1/responses",{method:"POST",headers:{authorization:`Bearer ${key}`,"content-type":"application/json"},body:JSON.stringify({model:input.model.gatewayModelId,instructions:input.instructions,input:input.messages,max_output_tokens:input.maxOutputTokens}),signal:controller.signal});
    ...
    if(!text) throw new ModelExecutionError("provider","model_output_empty");
    ...
    return {text,usage,requestId:response.headers.get("x-request-id")??(String(value.id??"")||null),durationMs:Math.round(performance.now()-started)};
  } catch(error) {
