...
export async function persistWebsiteSourceRecords(attempt:WebsiteCrawlAttemptRecord,documents:WebsiteSourceDocumentRecord[],blocks:WebsiteSourceBlockRecord[]):Promise<void>{
  ...
  for(const document of documents){
    const matches=await sql`SELECT EXISTS(
      SELECT 1 FROM ai_builder_website_source_documents
      WHERE id=${document.id}
        AND schema_version=${document.schemaVersion}
        AND crawl_attempt_id=${document.crawlAttemptId}
        AND actual_fetched_url=${document.actualFetchedUrl}
        AND canonical_url IS NOT DISTINCT FROM ${document.canonicalUrl}
        AND redirect_chain=${JSON.stringify(document.redirectChain)}::jsonb
        AND source_type=${document.sourceType}
        AND content_type=${document.contentType}
        AND status=${document.status}
        AND fetched_at=${document.fetchedAt}::timestamptz
        AND source_content_hash=${document.sourceContentHash}
        AND extracted_content_hash=${document.extractedContentHash}
        AND language IS NOT DISTINCT FROM ${document.language}
        AND source_truncated=${document.sourceTruncated}
        AND extraction_truncated=${document.extractionTruncated}
        AND discovery_method=${document.discoveryMethod}
        AND discovered_from_url IS NOT DISTINCT FROM ${document.discoveredFromUrl}
    ) AS matches` as unknown as Array<{matches:boolean}>;
    if(!matches[0]?.matches)collision("source_document",document.id);
  }
  ...
  for(const block of blocks){
    const matches=await sql`SELECT EXISTS(
      SELECT 1 FROM ai_builder_website_source_blocks
      WHERE id=${block.id}
        AND schema_version=${block.schemaVersion}
        AND source_document_id=${block.sourceDocumentId}
        AND crawl_attempt_id=${block.crawlAttemptId}
        AND block_type=${block.type}
        AND normalized_text=${block.normalizedText}
        AND coordinates=${JSON.stringify(block.coordinates)}::jsonb
        AND extraction_method=${block.extractionMethod}
    ) AS matches` as unknown as Array<{matches:boolean}>;
    if(!matches[0]?.matches)collision("source_block",block.id);
  }
}
