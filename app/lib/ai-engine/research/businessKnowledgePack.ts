...
export async function runBusinessWebsiteResearchRequest(request: Request, options: { internalWorker: boolean }) {
  const { internalWorker } = options;
      ...
      try {
        ...
        send({
          ...
          usage: {
            model,
            aiCalls,
            extractionUnits: aiExtractionUnits,
            tokens: usage,
            estimatedCostUsd: hasCostEstimate ? {
              input: estimatedInputCostUsd,
              output: estimatedOutputCostUsd,
              total: estimatedInputCostUsd + estimatedOutputCostUsd,
            } : null,
          },
        });
      } catch (error) {
