...
export function buildBusinessMemory(input: BuildBusinessMemoryInput): BusinessMemory {
  ...
  return { id: stableId("business_memory", [input.session.id]), schemaVersion: 1, projectId: input.session.id, entities: sortById(entities), assertions: sortById(assertions), relationships: sortById(relationships), sources: sortById(Array.from(sources.values())), evidence: sortById(Array.from(evidence.values())), ...(entityMerges ? { entityMerges } : {}), createdAt: input.session.createdAt, updatedAt: latest([input.session.updatedAt, input.websiteKnowledge?.imported_at ?? "", ...assertions.map((assertion) => assertion.updatedAt)]) };
}
