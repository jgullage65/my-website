...
function operationFor(category: DriftCategory): TypedRepairOperation["type"] { switch (category) {
 ...
 case "source_evidence_lineage_mismatch": return "repairEvidenceLink"; case "trusted_knowledge_revision_mismatch": return "synchronizeTrustedKnowledgeRevision"; case "revision_fingerprint_mismatch": return "synchronizeFingerprint";
...
export function buildTypedRepairPlan(drift: DriftItem[]): TypedRepairOperation[] { if (drift.some(x => !x.repairable)) throw new Error("business_memory_unrepairable_drift"); return drift.map(item => ({ type: operationFor(item.category), model: item.model, id: item.id, payload: item.expected })).sort((a,b) => ranks[a.type] - ranks[b.type] || a.id.localeCompare(b.id)); }
