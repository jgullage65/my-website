...
function operationFor(category: DriftCategory): TypedRepairOperation["type"] { switch (category) {
 ...
 case "missing_alias": return "insertAlias"; case "stale_alias": return "updateAlias";
 ...
 case "source_evidence_lineage_mismatch": return "repairEvidenceLink"; case "governance_history_gap": case "governance_history_integrity_failure": throw new Error("business_memory_unrepairable_drift"); case "trusted_knowledge_revision_mismatch": return "synchronizeTrustedKnowledgeRevision"; case "revision_fingerprint_mismatch": return "synchronizeFingerprint";
...
export function buildTypedRepairPlan(drift: DriftItem[]): TypedRepairOperation[] { if (drift.some(x => !x.repairable)) throw new Error("business_memory_unrepairable_drift"); return drift.map(item => ({ type: item.model === "sources" ? "repairSourceLink" : item.model === "conflictAssertionLinks" ? "replaceConflictAssertionLinks" : item.model === "assertionSourceLinks" ? "repairSourceLink" : item.model === "assertionEvidenceLinks" || item.model === "evidence" ? "repairEvidenceLink" : operationFor(item.category), model: item.model, id: item.id, payload: item.expected })).sort((a,b) => ranks[a.type] - ranks[b.type] || a.id.localeCompare(b.id)); }
