 ...
 */
export function extractBusinessMemoryProvenance(row: Row): { origin: KnowledgeSourceOrigin; sourceEntryId: string | null; intakeBlockId: string | null; url: string | null; label: string | null; capturedAt: string; crawlAttemptId: string | null; sourceDocumentId:string|null;sourceBlockId:string|null } {
...
export async function rebuildPersistedBusinessMemoryFromTrustedKnowledge(client: QueryClient, projectId: string, trustedKnowledgeRevision: number, options: { forceReconstruction?: boolean } = {}): Promise<void> {
  ...
  if (unchanged) {
    ...
    // identical material state must retain its material revision.
    await client.query("UPDATE ai_builder_business_memory SET trusted_knowledge_revision=$2, updated_at=$3::timestamptz WHERE project_id=$1", [projectId, trustedKnowledgeRevision, now]);
  } else {
  ...
  for (const row of trusted.rows as Row[]) {
    ...
    for (const item of byClaim.get(row.claim_id) ?? []) { const p = extractBusinessMemoryProvenance(item), sid = `business_memory_source:${item.canonical_source_id}`, evid = `business_memory_evidence:${item.evidence_id}`; sources.add(sid); evidences.add(evid); await client.query(`INSERT INTO ai_builder_business_memory_sources (id,memory_id,canonical_source_id,origin,source_entry_id,intake_block_id,url,label,captured_at,crawl_attempt_id,source_document_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::timestamptz,$10,$11) ON CONFLICT (id) DO UPDATE SET origin=EXCLUDED.origin,source_entry_id=EXCLUDED.source_entry_id,intake_block_id=EXCLUDED.intake_block_id,url=EXCLUDED.url,label=EXCLUDED.label,captured_at=EXCLUDED.captured_at,crawl_attempt_id=EXCLUDED.crawl_attempt_id,source_document_id=EXCLUDED.source_document_id`, [sid, rootId, item.canonical_source_id, p.origin, p.sourceEntryId, p.intakeBlockId, p.url, p.label, p.capturedAt, p.crawlAttemptId,p.sourceDocumentId]); await client.query(`INSERT INTO ai_builder_business_memory_evidence (id,memory_id,source_id,canonical_evidence_id,excerpt,url,captured_at,source_block_id) VALUES ($1,$2,$3,$4,$5,$6,$7::timestamptz,$8) ON CONFLICT (id) DO UPDATE SET excerpt=EXCLUDED.excerpt,url=EXCLUDED.url,captured_at=EXCLUDED.captured_at,source_block_id=EXCLUDED.source_block_id`, [evid, rootId, sid, item.evidence_id, item.content, item.evidence_url, item.captured_at,p.sourceBlockId]); await client.query("INSERT INTO ai_builder_business_memory_assertion_sources (assertion_id,source_id) VALUES ($1,$2) ON CONFLICT DO NOTHING", [aid, sid]); await client.query("INSERT INTO ai_builder_business_memory_assertion_evidence (assertion_id,evidence_id) VALUES ($1,$2) ON CONFLICT DO NOTHING", [aid, evid]); }
  }
  const relationships: string[] = []; for (const relationship of state.relationships) { const from = assertionByKey.get(canonicalJson(relationship.from)), to = assertionByKey.get(canonicalJson(relationship.to)); if (!from || !to) continue; const id = stableRelationshipId(relationship); relationships.push(id); await client.query(`INSERT INTO ai_builder_business_memory_relationships (id,memory_id,relationship_type,from_entity_id,to_entity_id,from_assertion_id,to_assertion_id,source_entry_ids,review_state,created_at,updated_at) SELECT $1,$2,'supports',a.entity_id,b.entity_id,$3,$4,$5::jsonb,b.review_state,b.created_at,b.updated_at FROM ai_builder_business_memory_assertions a JOIN ai_builder_business_memory_assertions b ON b.id=$4 WHERE a.id=$3 ON CONFLICT (id) DO UPDATE SET from_entity_id=EXCLUDED.from_entity_id,to_entity_id=EXCLUDED.to_entity_id,from_assertion_id=EXCLUDED.from_assertion_id,to_assertion_id=EXCLUDED.to_assertion_id,source_entry_ids=EXCLUDED.source_entry_ids,review_state=EXCLUDED.review_state,updated_at=EXCLUDED.updated_at`, [id, rootId, from, to, JSON.stringify(relationship.sourceEntryIds)]); }
