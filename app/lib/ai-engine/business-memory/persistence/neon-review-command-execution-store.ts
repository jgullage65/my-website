  ...
  async transaction<T>(_commandId: string, operation: (transaction: ReviewCommandExecutionTransaction) => Promise<T>): Promise<T> {
    return operation({
      ...
      updateReviewReadModels: async () => {
        const counts = (await this.client.query("SELECT COUNT(*) FILTER (WHERE status IN ('approved','corrected')) AS approved, COUNT(*) FILTER (WHERE status = 'proposed') AS proposed, COUNT(*) FILTER (WHERE status = 'archived') AS archived, COUNT(*) FILTER (WHERE item_kind = 'context_entry' AND status IN ('approved','corrected')) AS approved_business_knowledge, COUNT(*) AS total FROM (SELECT c.status, 'context_entry' AS item_kind FROM ai_builder_context_entries c WHERE c.project_id = $1 AND NOT EXISTS (SELECT 1 FROM ai_builder_faq_entries f WHERE f.project_id = c.project_id AND f.source_entry_ids ? c.id) UNION ALL SELECT status, 'faq' AS item_kind FROM ai_builder_faq_entries WHERE project_id = $1) review_items", [this.projectId])).rows[0];
        const categories = (await this.client.query("SELECT c.category, COUNT(*) AS total FROM ai_builder_context_entries c WHERE c.project_id = $1 AND NOT EXISTS (SELECT 1 FROM ai_builder_faq_entries f WHERE f.project_id = c.project_id AND f.source_entry_ids ? c.id) GROUP BY c.category", [this.projectId])).rows;
