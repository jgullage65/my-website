...
/** Persists at most one event per mismatch identity and hour via a partial unique index. */
export async function writeRuntimeAuthorityMismatchAfterRollback(projectId:string, errorCode:string, artifactFingerprint:string|null, occurredAt=new Date()):Promise<void>{
  ...
  const deduplicationKey=[projectId,errorCode,artifactFingerprint??"missing",bucket].join("\0");
  try{await ensureAiBuilderSchema();const client=await (pool??=new Pool({connectionString:process.env.DATABASE_URL})).connect();try{await client.query("BEGIN");await writeOperationalEvent(client,{projectId,eventType:"runtime_authority_mismatch",category:"runtime_cutover",severity:"error",outcome:"rejected",sourceComponent:"loadCanonicalRuntimeKnowledge",assistantProjectionFingerprint:artifactFingerprint??undefined,errorCode,errorMessage:"Canonical runtime validation failed.",metadata:{hourBucket:bucket,artifactMissing:artifactFingerprint===null},deduplicationKey});await client.query("COMMIT");}catch(error){await client.query("ROLLBACK").catch(()=>undefined);throw error;}finally{client.release();}}catch(error){console.error("runtime authority mismatch telemetry failed",error);}
}
