...
export async function inspectProjectDrift(client: QueryClient, projectId: string) {
 ...
 if(m&&computedFingerprint!==m.state_fingerprint) findings.push({code:"business_memory_fingerprint_mismatch",expectedValue:computedFingerprint,observedValue:m.state_fingerprint,severity:"error",repairability:"repairable"});
 ...
 if(!migrationReady) findings.push({code:"migration_not_ready",expectedValue:"business_memory_backfilled",observedValue:p?.migration_state??null,severity:"warning",repairability:"repairable"});
 if(parityRow?.active_runtime_authority!==p?.runtime_authority) findings.push({code:"parity_authority_mismatch",expectedValue:p?.runtime_authority??null,observedValue:parityRow?.active_runtime_authority??null,severity:"error",repairability:"blocked"});
 if (!parityRow || parityRow.status!=="MATCH" || parityRow.artifact_fingerprint!==a?.business_memory_fingerprint) findings.push({code:"runtime_parity_mismatch",expectedValue:"MATCH active artifact",observedValue:parityRow?.status??null,severity:"error",repairability:"blocked"});
 ...
 if(previousSignature&&previousSignature!==signature) await writeOperationalEvent(client,{projectId,eventType:"drift_resolved",category:"drift",severity:"info",outcome:"resolved",sourceComponent:"inspectProjectDrift",metadata:{signature:previousSignature}});
 ...
 return {healthy:findings.length===0,drifted:findings.length>0,blocked:findings.some(f=>f.repairability==="blocked"),migration_pending:!migrationReady,findings};
}
