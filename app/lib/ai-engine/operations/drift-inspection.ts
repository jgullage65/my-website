...
export async function inspectProjectDrift(client: QueryClient, projectId: string) {
 ...
 if(previousSignature&&!findings.length) await writeOperationalEvent(client,{projectId,eventType:"drift_resolved",category:"drift",severity:"info",outcome:"resolved",sourceComponent:"inspectProjectDrift",metadata:{signature:previousSignature,resolutionReason:"cleared"}});
 if(findings.length&&previousSignature!==signature) { if(previousSignature) await writeOperationalEvent(client,{projectId,eventType:"drift_resolved",category:"drift",severity:"info",outcome:"resolved",sourceComponent:"inspectProjectDrift",metadata:{signature:previousSignature,resolutionReason:"superseded",replacementSignature:signature}}); const summary=Object.fromEntries(findings.reduce<Map<string,number>>((out,f)=>out.set(f.code,(out.get(f.code)??0)+1),new Map())); await writeOperationalEvent(client,{projectId,eventType:"drift_detected",category:"drift",severity:"warning",outcome:"detected",sourceComponent:"inspectProjectDrift",metadata:{findingCount:findings.length,signature,summary}}); await writeOperationalEvent(client,{projectId,eventType:"drift_unresolved",category:"drift",severity:"warning",outcome:"detected",sourceComponent:"inspectProjectDrift",metadata:{findingCount:findings.length,signature,summary}}); }
