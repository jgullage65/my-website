    ...
    <div className="overflow-hidden bg-[#030713] text-white">
      ...
      <Section className="mx-auto grid max-w-[94rem] gap-10 border-b border-[rgba(212,175,55,.10)] px-5 py-20 sm:px-8 lg:grid-cols-[.88fr_1.12fr] lg:px-10 lg:py-20">
        <motion.div variants={fadeUp} className="self-center text-center lg:text-left">
          <p className="text-center text-xs font-black uppercase tracking-[.32em] text-[var(--gold)] lg:text-left">AI Systems</p>
          <h2 className="mt-4 text-center text-4xl font-black leading-[.96] tracking-[-.055em] sm:text-6xl lg:text-left">Smarter systems.<br /><span className="text-[var(--gold)]">Stronger businesses.</span></h2>
          <div role="tablist" aria-label="AI system capabilities" onKeyDown={onTabKeyDown} className="mt-8 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap justify-center lg:justify-start">
          ...
          <AnimatePresence mode="wait"><motion.div key={current.id} id={`${tabRoot}-${current.id}-panel`} role="tabpanel" aria-labelledby={`${tabRoot}-${current.id}-tab`} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: .3 }} className="mt-8"><h3 className="text-2xl font-black tracking-[-.035em]">{current.heading}</h3><p className="mx-auto mt-4 max-w-xl leading-7 text-[var(--muted)] lg:mx-0">{current.description}</p><ul className="mx-auto mt-6 grid w-fit gap-3 text-left lg:mx-0">{current.lines.map(line => <li key={line} className="flex gap-3 text-sm font-bold text-slate-200"><span className="mt-1.5 h-2 w-2 rounded-full bg-[var(--gold)] shadow-[0_0_18px_rgba(212,175,55,.6)]" />{line}</li>)}</ul></motion.div></AnimatePresence>
          <div className="mt-8 flex justify-center lg:justify-start"><GoldButton href="/ai-tools">Explore AI Systems →</GoldButton></div>
        </motion.div>
