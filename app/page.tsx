...
function ProductMedia({ src, alt, label, className = "", ratio = "aspect-[16/10]" }: { src?: string; alt: string; label: string; className?: string; ratio?: string }) {
  const isLeadForgePreview = alt === "LeadForge premium website";
  const shouldContain = alt === "Integration architecture preview" || alt === "Custom business software" || alt === "AI Systems showcase" || alt === "Custom Software showcase";
  return (
    <div className={`group relative overflow-hidden rounded-[1.15rem] border border-[rgba(212,175,55,.18)] bg-[#030711] shadow-[0_28px_90px_rgba(0,0,0,.48),0_0_0_1px_rgba(255,255,255,.035)_inset] ${ratio} ${className}`}>
      {src ? <img src={src} alt={alt} className={`h-full w-full ${isLeadForgePreview || shouldContain ? "object-contain" : "object-cover"}`} /> : null}
      {!src ? (
...
export default function HomePage() {
  ...
  return (
    <div className="overflow-hidden bg-[#030713] text-white">
      <Section className="relative mx-auto grid max-w-[94rem] gap-10 px-0 pb-14 pt-0 sm:px-8 sm:py-16 lg:grid-cols-[.9fr_1.1fr] lg:px-10 lg:py-20">
        ...
        <motion.div variants={fadeUp} className="relative z-10 order-2 self-center px-5 text-center sm:px-0 lg:order-1">
        ...
        <motion.div variants={mediaIn} className="relative z-10 order-1 self-center lg:order-2">
          ...
          <ProductMedia
            ...
            label="JG Creative Studio"
            ratio="aspect-[16/11]"
            className="rounded-none border-x-0 sm:rounded-[1.6rem] sm:border-x"
          />
        </motion.div>
      </Section>
          ...
          {workCards.map(([title, desc, cta, href, label]) => (
            <motion.article variants={fadeUp} whileHover={{ y: -8, rotateX: 2 }} key={title} className="rounded-[1.45rem] border border-[rgba(212,175,55,.13)] bg-[linear-gradient(145deg,rgba(9,16,32,.94),rgba(2,5,14,.98))] p-4 shadow-[0_30px_90px_rgba(0,0,0,.36)]">
              <ProductMedia
                ...
                label={label}
                ratio={title === "Premium Websites" ? "aspect-[1217/521]" : "aspect-[16/11]"}
              />
              <div className="p-2 pt-5 text-center">
                ...
                <Link href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noopener noreferrer" : undefined} className="cta-raised mt-6 inline-flex items-center justify-center rounded-lg border border-amber-300/15 bg-[#081226] px-5 py-3 text-sm font-black text-white transition duration-300 hover:-translate-y-0.5 hover:bg-[#0b1830]">{cta} →</Link>
              </div>
            </motion.article>
          ))}
        </div>
